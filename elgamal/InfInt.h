...
public:
    ...
    InfInt getBit(int n);
    InfInt powmod(InfInt e, InfInt mod);
...
inline InfInt InfInt::getBit(int n)
{
    InfInt _2k = 1;
    while (n--) _2k *= 2; 
    std::cout << "_2k = " << _2k << '\n';
    return (*this / _2k) - (*this / (_2k * 2) * 2);
} 

inline InfInt InfInt::powmod(InfInt e, InfInt mod)
{
    if (mod == 1) 
        return 0;
    InfInt base = (*this) % mod;
    InfInt result = 1;
    while (e > 0) {
        if (e % 2 == 1) 
